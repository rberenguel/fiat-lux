...
export function getRestartButtonText(restartCount) {
  switch (restartCount) {
    ...
    default:
      // Fifth restart and beyond - deterministic selection based on restart count
      // This ensures the same restart always shows the same name
      const index = (restartCount - 4) % RESTART_NAMES.length;
      const selected = RESTART_NAMES[index];
      return {
        main: selected.name,
